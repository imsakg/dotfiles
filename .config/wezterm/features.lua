...
M.set_appearance = function(appearance)
	local lua = M.getLuaFromTOML()
	lua.appearance = appearance
	M.writeLuaToTOML(lua)
end

M.get_theme = function()
	local lua = M.getLuaFromTOML()
	return lua.appearance == "Dark" and lua.colorscheme_dark or lua.colorscheme_light
end
...
M.theme_switcher = function(window, pane)
	...
	local action = wezterm.action_callback(function(_, _, _, label)
		if label then
			local lua = M.getLuaFromTOML()
			if lua.appearance == "Dark" then
				lua.colorscheme_dark = label
			else
				lua.colorscheme_light = label
			end
			M.writeLuaToTOML(lua)
		end
	end)
